...
def case02_verify():
    for i, privkey in enumerate(PRIVKEYS):
        for message in MESSAGES:
            ...
            assert bls._Verify(pubkey, message, signature)
            ...
            assert not bls._Verify(wrong_pubkey, message, signature)
            ...
            identifier = f'{encode_hex(pubkey)}_{encode_hex(message)}'
            assert not bls._Verify(pubkey, message, tampered_signature)
        ...
        assert bls._Verify(Z1_PUBKEY, message, Z2_SIGNATURE)
...
def case04_fast_aggregate_verify():
    for i, message in enumerate(MESSAGES):
        ...
        assert bls._FastAggregateVerify(pubkeys, message, aggregate_signature)
        ...
        assert not bls._FastAggregateVerify(pubkeys_extra, message, aggregate_signature)
        ...
        identifier = f'{pubkeys_serial}_{encode_hex(message)}'
        assert not bls._FastAggregateVerify(pubkeys, message, tampered_signature)
    ...
    assert not bls._FastAggregateVerify([], message, Z2_SIGNATURE)
    ...
    assert not bls._FastAggregateVerify([], message, NO_SIGNATURE)
...
def case05_aggregate_verify():
    ...
    tampered_signature = aggregate_signature[:4] + b'\xff\xff\xff\xff'
    assert not bls._AggregateVerify(pubkey, messages, tampered_signature)
    ...
    # Invalid pubkeys and signature -- len(pubkeys) == 0 and signature == Z1_SIGNATURE
    assert not bls._AggregateVerify([], [], Z2_SIGNATURE)
    ...
    # Invalid pubkeys and signature -- len(pubkeys) == 0 and signature == 0x00...
    assert not bls._AggregateVerify([], [], NO_SIGNATURE)
