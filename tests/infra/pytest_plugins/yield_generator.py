...
# Stash key for storing manifest on pytest.Function items.
_manifest_key: StashKey[Manifest] = StashKey()
...
def _infer_manifest(item: pytest.Function) -> Manifest | None:
    """Infer manifest fields from the test's file path and runner config."""
    path = item.parent.path
    module_name = path.stem

    result = _find_runner(path)
    if result is None:
        return None
    pkg_name, config = result

    runner_name = config.get("runner_name", pkg_name)
    handler_name = _derive_handler_name(config, module_name)
    suite_name = config.get("suite_name", getattr(item.obj, "suite_name", "pyspec_tests"))
    ...
    if hasattr(item.obj, "manifest") and item.obj.manifest is not None:
        manifest = item.obj.manifest.with_defaults(manifest)
...
class YieldGeneratorPlugin:
    ...
    def pytest_pyfunc_call(self, pyfuncitem: pytest.Function):
        manifest = pyfuncitem.stash.get(_manifest_key, None)
        if manifest is None:
            return None
    ...
    def pytest_collection_modifyitems(self, config, items):
        ...
        for item in items:
            if isinstance(item, pytest.Function):
                manifest = _infer_manifest(item)
                if manifest is not None:
                    item.stash[_manifest_key] = manifest
