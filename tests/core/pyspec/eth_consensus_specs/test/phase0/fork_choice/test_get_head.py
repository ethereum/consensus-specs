...
def test_split_tie_breaker_no_attestations(spec, state):
    ...
    time_ms = store.genesis_time_ms + (block_2.slot + 1) * spec.config.SLOT_DURATION_MS
...
def test_filtered_block_tree(spec, state):
    ...
    ) * spec.config.SLOT_DURATION_MS + store.genesis_time_ms
    on_tick_and_append_step(spec, store, current_time_ms, test_steps)
...
def test_proposer_boost_correct_head(spec, state):
    ...
    time_ms = store.genesis_time_ms + block_1.slot * spec.config.SLOT_DURATION_MS
    on_tick_and_append_step(spec, store, time_ms, test_steps)
    ...
    time_ms = store.genesis_time_ms + (block_1.slot + 1) * spec.config.SLOT_DURATION_MS
    on_tick_and_append_step(spec, store, time_ms, test_steps)
    assert store.proposer_boost_root == spec.Root()
    check_head_against_root(spec, store, spec.hash_tree_root(block_2))
    output_head_check(spec, store, test_steps)
...
def test_discard_equivocations_on_attester_slashing(spec, state):
    ...
    time_ms = store.genesis_time_ms + (block_eqv.slot + 2) * spec.config.SLOT_DURATION_MS
    on_tick_and_append_step(spec, store, time_ms, test_steps)
...
def test_discard_equivocations_slashed_validator_censoring(spec, state):
    ...
    current_time_ms = anchor_state.slot * spec.config.SLOT_DURATION_MS + store.genesis_time_ms
    on_tick_and_append_step(spec, store, current_time_ms, test_steps)
    ...
    current_time_ms = store.genesis_time_ms + (block_1.slot + 1) * spec.config.SLOT_DURATION_MS
    on_tick_and_append_step(spec, store, current_time_ms, test_steps)
