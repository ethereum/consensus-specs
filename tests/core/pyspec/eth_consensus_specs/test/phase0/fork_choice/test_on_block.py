...
def test_proposer_boost(spec, state):
    ...
    time_ms = (
        spec.seconds_to_milliseconds(store.genesis_time)
        + block.slot * spec.config.SLOT_DURATION_MS
        + spec.seconds_to_milliseconds(late_block_cutoff - 1)
    )
    ...
    time_ms = (
        spec.seconds_to_milliseconds(store.genesis_time) + block.slot * spec.config.SLOT_DURATION_MS
    )
    on_tick_and_append_step(spec, store, time_ms, test_steps)
    yield from add_block(spec, store, signed_block, test_steps)
    assert store.proposer_boost_root == spec.hash_tree_root(block)
...
def test_proposer_boost_root_same_slot_untimely_block(spec, state):
    ...
    time_ms = (
        spec.seconds_to_milliseconds(store.genesis_time)
        + block.slot * spec.config.SLOT_DURATION_MS
        + spec.seconds_to_milliseconds(late_block_cutoff)
    )
...
def test_proposer_boost_is_first_block(spec, state):
    ...
    time_ms = (
        spec.seconds_to_milliseconds(store.genesis_time)
        + block_a.slot * spec.config.SLOT_DURATION_MS
        + spec.seconds_to_milliseconds(late_block_cutoff - 1)
    )
