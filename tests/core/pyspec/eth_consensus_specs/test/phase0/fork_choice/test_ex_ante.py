...
def test_ex_ante_vanilla(spec, state):
    ...
    # Block C received at N+2 — C is head
    time_ms = state_c.slot * spec.config.SLOT_DURATION_MS + spec.seconds_to_milliseconds(
        store.genesis_time
    )
...
def test_ex_ante_sandwich_without_attestations(spec, state):
    ...
    time_ms = state_d.slot * spec.config.SLOT_DURATION_MS + spec.seconds_to_milliseconds(
        store.genesis_time
    )
    on_tick_and_append_step(spec, store, time_ms, test_steps)
