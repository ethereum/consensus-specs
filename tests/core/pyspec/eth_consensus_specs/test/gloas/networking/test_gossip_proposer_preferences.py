...
def test_gossip_proposer_preferences__ignore_pre_gloas_epoch(spec, state):
    ...
    time_ms = spec.compute_time_at_slot_ms(store.genesis_time, spec.Slot(0))
    yield "current_time_ms", "meta", int(time_ms)
    messages = []
...
def test_gossip_proposer_preferences__ignore_slot_before_lookahead(spec, state):
    ...
    time_ms = spec.compute_time_at_slot_ms(store.genesis_time, epoch_start_slot)
    yield "current_time_ms", "meta", int(time_ms)
    messages = []
...
def test_gossip_proposer_preferences__ignore_outside_lookahead_disparity(spec, state):
    ...
    time_ms = (
        spec.compute_time_at_slot_ms(store.genesis_time, lookahead_start_slot)
...
def test_gossip_proposer_preferences__valid_at_slot_start_disparity_edge(spec, state):
    ...
    time_ms = (
        spec.compute_time_at_slot_ms(store.genesis_time, proposal_slot)
        + spec.config.MAXIMUM_GOSSIP_CLOCK_DISPARITY
    )
    yield "current_time_ms", "meta", int(time_ms)
    messages = []
...
def test_gossip_proposer_preferences__valid_dependent_block_across_empty_epochs(spec, state):
    ...
    time_ms = spec.compute_time_at_slot_ms(store.genesis_time, validation_state.slot)
    yield "current_time_ms", "meta", int(time_ms)
    time_ms += 100
    result, reason = run_validate_gossip(
        spec,
