...
from eth_consensus_specs.test.helpers.eip8198.fork import (
...
@with_phases(phases=[HEZE], other_phases=[EIP8198])
@spec_test
@with_state
@with_meta_tags(EIP8198_FORK_TEST_META_TAGS)
def test_fork_base_state(spec, phases, state):
    yield from run_fork_test(phases[EIP8198], state)
