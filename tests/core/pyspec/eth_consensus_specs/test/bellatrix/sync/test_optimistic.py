...
def test_from_syncing_to_invalid(spec, state):
    ...
    for i in range(3):
        ...
        block.body.execution_payload.extra_data = spec.sha256_hash(bytes(f"chain_a_{i}", "UTF-8"))
    ...
    for i in range(3):
        ...
        block.body.execution_payload.extra_data = spec.sha256_hash(bytes(f"chain_b_{i}", "UTF-8"))
        block.body.execution_payload.block_hash = compute_el_block_hash(
            spec, block.body.execution_payload, state
        )
