...
def build_mock_builder(spec, i: int, balance: int):
    return spec.Builder(
        ...
        execution_address=spec.ExecutionAddress(spec.sha256_hash(builder_pubkeys[i])[12:]),
        balance=balance,
        deposit_epoch=0,
    ...
    if is_post_electra(spec):
        if balance > spec.MIN_ACTIVATION_BALANCE:
            ...
            withdrawal_credentials = (
                ...
                + b"\x00" * 11
                + spec.sha256_hash(withdrawal_pubkey)[12:]
            )
        else:
            # insecurely use pubkey as withdrawal key as well
            withdrawal_credentials = (
                spec.BLS_WITHDRAWAL_PREFIX + spec.sha256_hash(withdrawal_pubkey)[1:]
            )
