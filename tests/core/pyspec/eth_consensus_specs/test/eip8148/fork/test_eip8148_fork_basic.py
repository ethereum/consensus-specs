...
from eth_consensus_specs.test.helpers.eip8148.fork import (
...
@spec_test
@with_state
@with_meta_tags(EIP8148_FORK_TEST_META_TAGS)
def test_fork_base_state(spec, phases, state):
    yield from run_fork_test(phases[EIP8148], state)
...
@with_state
@with_meta_tags(EIP8148_FORK_TEST_META_TAGS)
def test_fork_compounding_validators(spec, phases, state):
    # Give some validators compounding credentials to verify that the upgrade
    # initializes their sweep thresholds to the maximum value
    for index in [0, 2, 4]:
        set_compounding_withdrawal_credential(spec, state, index)
    yield from run_fork_test(phases[EIP8148], state)
