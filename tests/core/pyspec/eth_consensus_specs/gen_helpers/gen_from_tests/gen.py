...
from eth_consensus_specs.test.helpers.typing import PresetBaseName, SpecForkName
...
def get_expected_modules(module, absolute=False):
    ...
    def _cached_walk_packages() -> list[ModuleInfo]:
        """Walk the packages in the eth_consensus_specs module. Cache the result for future calls."""
        global CACHED_WALK_PACKAGES

        if CACHED_WALK_PACKAGES is None:
            eth_consensus_specs = import_module("eth_consensus_specs")
            path = eth_consensus_specs.__path__
            prefix = eth_consensus_specs.__name__ + "."
