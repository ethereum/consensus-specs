...
def test_all_builder_withdrawals_invalid(spec, state):
    ...
    assert any(w.validator_index == 5 for w in withdrawals), (
...
def test_mixed_all_three_withdrawal_types(spec, state):
    ...
    builder_pos = next(
        (i for i, w in enumerate(withdrawals) if w.validator_index == builder_index), None
    )
    pending_pos = next(
        (i for i, w in enumerate(withdrawals) if w.validator_index == pending_index), None
    )
    regular_pos = next(
        (i for i, w in enumerate(withdrawals) if w.validator_index == regular_index), None
    )
...
def test_builder_max_capacity_no_room_others(spec, state):
    ...
    assert len(state.validators) >= num_builders + 2, (
        f"Test requires at least {num_builders + 2} validators"
    )
    ...
    builder_withdrawals = [w for w in withdrawals if w.validator_index < num_builders]
    assert len(builder_withdrawals) == num_builders, "All withdrawals should be builders only"
