...
def run_get_custody_columns(spec, peer_count, custody_subnet_count):
    ...
    assert custody_subnet_count <= spec.config.DATA_COLUMN_SIDECAR_SUBNET_COUNT
...
@spec_test
@single_phase
def test_custody_subnet_count_int_bitlength(spec):
    assert spec.config.DATA_COLUMN_SIDECAR_SUBNET_COUNT == uint8(spec.config.DATA_COLUMN_SIDECAR_SUBNET_COUNT)