...
def test_coset_fft(spec):
    ...
    coset_shift = spec.bls.Scalar(spec.PRIMITIVE_ROOT_OF_UNITY)
...
def test_construct_vanishing_polynomial(spec):
    ...
    for cell_index in range(spec.CELLS_PER_EXT_BLOB):
        ...
        if cell_index in unique_missing_cell_indices:
            assert all(a == spec.bls.Scalar(0) for a in zero_poly_eval_brp[start:end])
...
def test_multiply_polynomial_degree_overflow(spec):
    ...
    poly2_coeff = [spec.bls.Scalar(rng.randint(0, BLS_MODULUS - 1)) for _ in range(spec.FIELD_ELEMENTS_PER_BLOB + 1)]
