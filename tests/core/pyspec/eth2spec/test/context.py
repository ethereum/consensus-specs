...
from eth2spec.revoke import mainnet as spec_revoke_mainnet, minimal as spec_revoke_minimal
from eth2spec.utils import bls

from .exceptions import SkippedTest
from .helpers.constants import (
    PHASE0, ALTAIR, BELLATRIX, CAPELLA, EIP4844, REVOKE, SHARDING,
    MINIMAL, MAINNET,
...
def config_fork_epoch_overrides(spec, state):
    ...
    elif _check_current_version(spec, state, REVOKE):
        overrides['ALTAIR_FORK_EPOCH'] = spec.GENESIS_EPOCH
        overrides['BELLATRIX_FORK_EPOCH'] = spec.GENESIS_EPOCH
        overrides['REVOKE_FORK_EPOCH'] = spec.GENESIS_EPOCH
...
def is_post_revoke(spec):
    return spec.fork == REVOKE
...
with_revoke_and_later = with_all_phases_except([PHASE0, ALTAIR, BELLATRIX, CAPELLA, EIP4844])
