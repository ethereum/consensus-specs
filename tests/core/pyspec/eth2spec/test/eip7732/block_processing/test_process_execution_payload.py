...
def run_execution_payload_processing(spec, state, signed_envelope, valid=True, execution_valid=True):
    """
...
def prepare_execution_payload_envelope(spec, state, builder_index=None, slot=None,
                                     beacon_block_root=None, state_root=None,
                                     execution_payload=None, execution_requests=None,
                                     blob_kzg_commitments=None, valid_signature=True):
    """
    ...
    if state_root is None:
        ...
        payment = post_state.builder_pending_payments[spec.SLOTS_PER_EPOCH + state.slot % spec.SLOTS_PER_EPOCH]
        if payment.withdrawal.amount > 0:
            exit_queue_epoch = spec.compute_exit_epoch_and_update_churn(post_state, payment.withdrawal.amount)
        ...
        post_state.builder_pending_payments[spec.SLOTS_PER_EPOCH + state.slot % spec.SLOTS_PER_EPOCH] = spec.BuilderPendingPayment()
...
def setup_state_with_payload_header(spec, state, builder_index=None, value=None):
    ...
    if value > 0:
        ...
        state.builder_pending_payments[spec.SLOTS_PER_EPOCH + state.slot % spec.SLOTS_PER_EPOCH] = pending_payment
...
def test_process_execution_payload_valid(spec, state):
    ...
    pre_payment = state.builder_pending_payments[spec.SLOTS_PER_EPOCH + state.slot % spec.SLOTS_PER_EPOCH]
    ...
    cleared_payment = state.builder_pending_payments[spec.SLOTS_PER_EPOCH + state.slot % spec.SLOTS_PER_EPOCH]
...
def test_process_execution_payload_with_blob_commitments(spec, state):
    ...
    state.latest_execution_payload_header.blob_kzg_commitments_root = blob_kzg_commitments.hash_tree_root()
...
def test_process_execution_payload_wrong_blob_commitments_root(spec, state):
    ...
    state.latest_execution_payload_header.blob_kzg_commitments_root = original_blob_commitments.hash_tree_root()
...
def test_process_execution_payload_wrong_gas_limit(spec, state):
    ...
    execution_payload = build_empty_execution_payload(spec, state)
    execution_payload.block_hash = state.latest_execution_payload_header.block_hash
    execution_payload.gas_limit = state.latest_execution_payload_header.gas_limit + 1  # Wrong gas limit
    execution_payload.parent_hash = state.latest_block_hash
...
def test_process_execution_payload_max_blob_commitments_valid(spec, state):
    ...
    max_blob_commitments = [spec.KZGCommitment(b'\x42' * 48) for _ in range(spec.config.MAX_BLOBS_PER_BLOCK)]
    blob_kzg_commitments = spec.List[spec.KZGCommitment, spec.MAX_BLOB_COMMITMENTS_PER_BLOCK](max_blob_commitments)
...
def test_process_execution_payload_execution_engine_invalid(spec, state):
    ...
    yield from run_execution_payload_processing(spec, state, signed_envelope, valid=False, execution_valid=False)