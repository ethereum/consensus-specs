...
def test_deposit_and_bls_change(spec, state):
    ...
    assert (
        validator_credentials[12:] == signed_address_change.message.to_execution_address
    )
...
def test_exit_and_bls_change(spec, state):
    ...
    assert spec.is_fully_withdrawable_validator(
        validator, balance, validator.withdrawable_epoch
    )
...
def test_partial_withdrawal_in_epoch_transition(spec, state):
    ...
    set_validator_partially_withdrawable(
        spec, state, index, excess_balance=1000000000000
    )
...
def test_many_partial_withdrawals_in_epoch_transition(spec, state):
    ...
    assert (
        len(get_expected_withdrawals(spec, state)) == spec.MAX_WITHDRAWALS_PER_PAYLOAD
    )
...
def test_withdrawal_success_two_blocks(spec, state):
    pre_state, signed_block_1, pre_next_withdrawal_index = _perform_valid_withdrawal(
        spec, state
    )
...
def test_top_up_and_partial_withdrawable_validator(spec, state):
    ...
    if is_post_electra(spec):
        ...
        assert (
            state.pending_deposits[0].withdrawal_credentials
...
def _insert_validator(spec, state, balance):
    effective_balance = (
        balance if balance < spec.MAX_EFFECTIVE_BALANCE else spec.MAX_EFFECTIVE_BALANCE
    )
    validator_index = len(state.validators)
