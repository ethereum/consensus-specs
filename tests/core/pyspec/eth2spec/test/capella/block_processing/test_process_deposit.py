...
def test_validator_invalid_high_funding_after_withdrawal(spec, state):
    """
     ...
     Have added support for the increased Electra deposit amount
     """
    ...
    # Perform a check to see if the spec is Electra version
    if is_post_electra(spec):
        max_effective_balance = spec.MAX_EFFECTIVE_BALANCE_ELECTRA
    else:
        max_effective_balance = spec.MAX_EFFECTIVE_BALANCE

    # Make a top-up deposit, attempting to deposit double the max effective balance
    amount = max_effective_balance * 2
    deposit = prepare_state_and_deposit(spec, state, validator_index, amount, signed=True)
