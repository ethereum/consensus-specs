...
def test_on_block_outside_safe_slots_but_finality(spec, state):
    ...
    state, store, _ = yield from apply_next_slots_with_attestations(
        spec, state, store, 5, True, True, test_steps=test_steps)
    assert state.finalized_checkpoint.epoch == store.finalized_checkpoint.epoch == 2
    ...
    assert spec.get_current_slot(store) % spec.SLOTS_PER_EPOCH >= spec.SAFE_SLOTS_TO_UPDATE_JUSTIFIED
    yield from add_block(spec, store, signed_block, test_steps=test_steps)
...
def test_new_justified_is_later_than_store_justified(spec, state):
    ...
    # epoch 3
    fork_3_state, store, _ = yield from apply_next_epoch_with_attestations(
        spec, fork_3_state, True, True, test_steps=test_steps)
    assert fork_3_state.finalized_checkpoint.epoch == 0

    # epoch 4, attest the first 5 blocks
    fork_3_state, store, _ = yield from apply_next_slots_with_attestations(
        spec, fork_3_state, 5, True, True, test_steps=test_steps)
    assert fork_3_state.finalized_checkpoint.epoch == 0
