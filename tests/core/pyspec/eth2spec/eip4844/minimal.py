...
Polynomial = Vector[BLSFieldElement, FIELD_ELEMENTS_PER_BLOB]
...
config = Configuration(
    ...
    EIP4844_FORK_VERSION=Version('0x04000001'),
    EIP4844_FORK_EPOCH=Epoch(18446744073709551615),
)
...
def initialize_beacon_state_from_eth1(eth1_block_hash: Hash32,
    ...
    fork = Fork(
        previous_version=config.EIP4844_FORK_VERSION,  # [Modified in EIP-4844] for testing only
        current_version=config.EIP4844_FORK_VERSION,  # [Modified in EIP-4844]
        epoch=GENESIS_EPOCH,
    )
    state = BeaconState(
...
def compute_fork_version(epoch: Epoch) -> Version:
    ...
    if epoch >= config.EIP4844_FORK_EPOCH:
        return config.EIP4844_FORK_VERSION
...
def upgrade_to_eip4844(pre: capella.BeaconState) -> BeaconState:
    ...
    post = BeaconState(
        ...
        fork=Fork(
            previous_version=pre.fork.current_version,
            current_version=config.EIP4844_FORK_VERSION,  # [Modified in EIP4844]
            epoch=epoch,
        ),
        # History
        latest_block_header=pre.latest_block_header,
