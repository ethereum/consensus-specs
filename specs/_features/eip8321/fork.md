...
`compute_epoch_at_slot(state.slot) == EIP8321_FORK_EPOCH`, an irregular state
change is made to upgrade to EIP-8321.

The upgrade occurs after the completion of the inner loop of `process_slots`
that sets `state.slot` equal to `EIP8321_FORK_EPOCH * SLOTS_PER_EPOCH`.

Every validator starts unregistered, on the legacy BLS reveal path, and migrates
by broadcasting a `SignedRandaoCommitmentRegistration`.
