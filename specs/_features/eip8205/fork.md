...
- [Configs](#configs)
- [Fork to EIP-8205](#fork-to-eip-8205)

<!-- mdformat-toc end -->

## Introduction

This document describes the process of the EIP-8205 upgrade.
...
`compute_epoch_at_slot(state.slot) == EIP8205_FORK_EPOCH`, an irregular state
change is made to upgrade to EIP-8205.

The upgrade occurs after the completion of the inner loop of `process_slots`
that sets `state.slot` equal to `EIP8205_FORK_EPOCH * SLOTS_PER_EPOCH`.

The empty list is correct for a standalone activation: no historical execution
layer request can pre-populate the new field.
...
```python
def upgrade_to_eip8205(pre: heze.BeaconState) -> BeaconState:
    ...
    post = BeaconState(
        ...
        fork=Fork(
            ...
            # [New in EIP8205]
            current_version=EIP8205_FORK_VERSION,
            epoch=epoch,
        ),
