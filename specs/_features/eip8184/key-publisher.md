...
- [Key publisher activities](#key-publisher-activities)
  - [Observing sealed-transaction commitments](#observing-sealed-transaction-commitments)
  ...
  - [Releasing the `SealedTransactionKeyMessage`](#releasing-the-sealedtransactionkeymessage)
...
Key publishers may be the senders themselves (trustless
self-decryption), or independent parties that sign bundles on behalf of
multiple senders.
...
existence and encryption instructions out-of-band. Senders direct
sealed transactions to a key publisher by setting `ticket.key_publisher`
to its address (and, for bundles, by including the ticket in a bundle
the publisher signs).
...
### Observing sealed-transaction commitments

Upon observing a beacon block at slot `t`, a key publisher inspects
`block.body.signed_execution_payload_bid.message.sealed_transaction_commitments`
for any commitment whose `commitment_key` matches a key it holds. For
each such commitment, the publisher proceeds with the verification
steps below.
...
2. That the aggregate `gas_obligation` across *all* commitments in
   `bid.sealed_transaction_commitments` is within
   `compute_top_of_block_gas_limit(bid.gas_limit)`.
...
`bid.sealed_transaction_commitments` (as determined by
[`is_valid_sealed_transaction_commitment_ordering`](./beacon-chain.md#new-is_valid_sealed_transaction_commitment_ordering))
is consistent with the `max_preceding_commitments` field of the
underlying sealed-transaction ticket(s). If any committed ticket would
execute at a position exceeding its `max_preceding_commitments`, the
publisher MUST NOT release the key.
...
### Releasing the `SealedTransactionKeyMessage`

The publisher constructs and broadcasts a
`SealedTransactionKeyMessage` over the
`sealed_transaction_key_message` gossip topic, before
`get_sealed_transaction_key_message_due_ms()` milliseconds into slot
`t+1`.
...
- Set `commit_index` to the index of the publisher's commitment within
  `bid.sealed_transaction_commitments`.
- Set `decryption_keys` to the list of decryption keys for the
  executable members of the commitment, in bundle order filtered to
  `executable[i] == 1`. For a non-bundle commitment, `decryption_keys`
  is a single-element list.

*Note*: For a single sealed-transaction commitment whose sender chooses
to self-decrypt, the sender may itself release the
`SealedTransactionKeyMessage`, even if `ticket.key_publisher` is set to
another address.

## Liability for non-reveal

There is no in-protocol mechanism by which a key publisher assumes
liability for bundle non-reveal in this version of the encrypted
mempool. Liability for the unrefunded portion of `top_of_block_fee`
(`top_of_block_fee // TOP_OF_BLOCK_FEE_FRACTION`, with the remainder
refunded to the sender via the execution layer) is paid by the *sender*
by default.
