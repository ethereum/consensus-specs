...
new `inclusion_list_transactions` field of `PayloadAttributes`.

*Note*: The proposer should call `prepare_execution_payload` at
`PROPOSER_INCLUSION_LIST_CUT_OFF` into the slot with the
`inclusion_list_transactions` field of `PayloadAttributes` set to the inclusion
list transactions gathered up to `PROPOSER_INCLUSION_LIST_CUT_OFF` into the slot
to ensure the execution payload satisfies the inclusion list constraints.
...
def get_sync_committee_message(
    ...
    return SyncCommitteeMessage(
        slot=state.slot,
        beacon_block_root=get_attester_head(store, block_root),  # [Modified in EIP7805]
