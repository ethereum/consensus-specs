...
- [Protocols](#protocols)
  - [`ExecutionEngine`](#executionengine)
    - [New `notify_inclusion_lists`](#new-notify_inclusion_lists)
...
*Note*: The `notify_inclusion_lists` function is added to the `ExecutionEngine`
protocol to instantiate the inclusion list constraints validation.
...
def validate_inclusion_lists(
    ...
    return execution_engine.notify_inclusion_lists(
...
def on_inclusion_list(store: Store, signed_inclusion_list: SignedInclusionList) -> None:
    ...
    store_inclusion_list(inclusion_list_store, inclusion_list, is_before_view_freeze_deadline)
```
