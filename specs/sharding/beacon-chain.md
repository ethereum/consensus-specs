...
def process_shard_header(state: BeaconState, signed_header: SignedShardBlobHeader) -> None:
    ...
    require(committee_work.status.selector == SHARD_WORK_PENDING,
            'This data should still be pending.')

    current_headers: Sequence[PendingShardHeader] = committee_work.status.value
    header_root = hash_tree_root(header)
    require(header_root not in [pending_header.root for pending_header in current_headers],
            'This header should not yet be in the pending list.')
