...
def get_genesis_beacon_state(genesis_validator_deposits: List[Deposit],
    ...
    state = BeaconState(
        ...
        # Randomness and committees
        latest_randao_mixes=FixedSizeList([ZERO_HASH for _ in range(LATEST_RANDAO_MIXES_LENGTH)]),
        ...
        # Recent state
        latest_crosslinks=FixedSizeList([Crosslink(epoch=GENESIS_EPOCH, crosslink_data_root=ZERO_HASH) for _ in range(SHARD_COUNT)]),
        latest_block_roots=FixedSizeList([ZERO_HASH for _ in range(SLOTS_PER_HISTORICAL_ROOT)]),
        latest_state_roots=FixedSizeList([ZERO_HASH for _ in range(SLOTS_PER_HISTORICAL_ROOT)]),
        latest_active_index_roots=FixedSizeList([ZERO_HASH for _ in range(LATEST_ACTIVE_INDEX_ROOTS_LENGTH)]),
        latest_slashed_balances=FixedSizeList([0 for _ in range(LATEST_SLASHED_EXIT_LENGTH)]),
