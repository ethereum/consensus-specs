...
The fork choice rule for any shard is LMD GHOST using the proposal committee and crosslink committee currently assigned to that shard (for simplicity counting the case of the same validator in both committees as two separate votes), but instead of being rooted in the genesis it is rooted in the block referenced in the most recent accepted crosslink (ie. `state.crosslinks[shard].shard_block_root`). Only blocks whose `beacon_chain_ref` is the block in the main beacon chain at the specified `slot` should be considered (if the beacon chain skips a slot, then the block at that slot is considered to be the block in the beacon chain at the highest slot lower than a slot).
