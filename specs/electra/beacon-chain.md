...
- [Electra -- The Beacon Chain](#electra----the-beacon-chain)
...
def get_attesting_indices(state: BeaconState, attestation: Attestation) -> Set[ValidatorIndex]:
    ...
    for committee_index in committee_indices:
        # [Modified in Electra:EIP7549]
        assert committee_index < get_committee_count_per_slot(state, attestation.data.target.epoch)
        ...
        assert len(committee_attesters) > 0  # [Modified in Electra:EIP7549]
        output = output.union(committee_attesters)

        committee_offset += len(committee)

    assert len(attestation.aggregation_bits) == committee_offset  # [Modified in Electra:EIP7549]
    return output
```
