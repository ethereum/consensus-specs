...
*Note*: Clients will be able to remove the `Eth1Data` polling mechanism in an uncoordinated fashion once the transition period is finished. The transition period is considered to be finished when a network reaches the point where `state.eth1_deposit_index == state.deposit_requests_start_index`.
