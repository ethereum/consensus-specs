...
the expected block proposer for the assigned `slot` or (b) `ATTESTATION_DUE_MS`
milliseconds after the start of `slot` has transpired -- whichever comes
_first_.
...
channel (`beacon_aggregate_and_proof`) `AGGREGATE_DUE_MS` milliseconds after the
start of `slot` has transpired.
