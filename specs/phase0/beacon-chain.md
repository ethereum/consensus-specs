...
| `INACTIVITY_PENALTY_QUOTIENT` | `2**24` (= 16,777,216) | If the chain has lost finality for N epochs, then each offline validator loses `N/INACTIVITY_PENALTY_QUOTIENT` of their balance during that epoch. When `N**2/2 = INACTIVITY_PENALTY_QUOTIENT`, the offline validators. The goal is to make it unambiguously not profitable to censor >1/3 lose `~1-1/e ~= 63.2%` of their balance. That happens after `~2**12.5` ~= `5792` epochs, or ~25 days. |
