...
* Let `epoch_boundary_block = head_block if epoch_start_slot == head_state.slot else get_block(backend, get_block_root(state, epoch_start_slot))` where `get_block` fetches a block from the backend.
